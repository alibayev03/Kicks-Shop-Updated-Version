...
function display_checkout(data) {
  ...
  for (let index in cart_items) {
    ...
    html += `<div class="checkout-item-list">
                  ...
                  <h6 id="price">UZS${price}</h6>
                </div>`;
  }

  total_price_html += `<div class="checkout-total">
                            <p class="checkout-text">yetkazib berish narxi: </p>
                            <p>UZS${data["delivery_charge"]}</p>
                          </div>
                          <div class="checkout-total">
                            <p class="checkout-text">QQS (5%): </p>
                            <p>UZS${data["vat"]}</p>
                          </div>
                          <div class="checkout-total">
                            <p class="checkout-text">umumiy narx (QQS, shu jumladan): </p>
                            <p>UZS${data["total_price"]}</p>
                          </div>`;
  delivery_html += `<tr><th>Ismingiz</th>
                        <td>${data["full_name"]}</td></tr>
                      <tr><th>Telefon Raqamingiz: </th>
...
function card_details() {
  if ($("#payment_type").val() == "card") {
    let html = "";
    html += `<tr class="card_detail_collapsible"><th>Karta tafsilotlari: </th>
               <td><input class"card_number" type="text" placeholder="Karta raqam"></td></tr>
               <tr class="card_detail_collapsible"><td></td>
                <td>
                  <input class="card_extra_info" type="text" placeholder="Tugash muddati">
