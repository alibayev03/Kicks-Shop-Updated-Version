function add_product_admin() {
  let html = `<div class="add-product">
    <h2><i class="fa-solid fa-plus"></i> Mahsulotlarni qo'shing</h2>
    <div class="add-form">
      <div class="form-row">
        <input type="text" id="brand" placeholder="Brendni kiriting" />
        <select id="category">
          <option hidden selected value="">Kategoriyani tanlang</option>
      ...
      <div class="form-row">
        <input type="number" id="price" placeholder="Narxni kiriting" />
        <input
          type="number"
          id="quantity"
      ...
      <div class="form-row">
        ...
        <button id="discard-product-btn">
          <i class="fa-solid fa-trash"></i> Bekor qilish
        </button>
      </div>
    </div>
  </div>`;
...
function display_users(data) {
  let html =
    "<h2><i class='fa-solid fa-trash'></i> Delete User Accounts</h2> <table class='table table-warning table-striped admin-product-table'><tr><th>User ID</th><th>Ismingiz</th><th>Familiyangiz</th><th>Address</th><th>Date of Birth</th><th>Gender</th><th>Email</th><th>Phone Number</th><td></td></tr>";
  for (let index in data) {
    let row = data[index];
