<?php
    ...
    //intializing the request type and the final result
    ...
    // admin function to delete a user from the database
    ...
    // getting the user that will be displayed in the manage users tab of the admin dashborad
    ...
    // changing the discount of a product in the admin dashboard
    ...
    // chaging the product quantity in the admin dashbooard
    ...
    // manage the quantities of the products from the admin dashboard
    ...
    // deleting a product from the admin dashboard
    ...
    //getting all the info about the products to display it in the admin dashboard
    ...
    // changing the address of the user
    ...
    //checking out the order of the user
...
function register($connection) {
    if (
        ...
        if ($count == 0) {
            ...
            $phone_number = 0; // если нужно, замени на $_POST["phone_number"]

            $stmt = $connection->prepare(
                "INSERT INTO Users (LastName, FirstName, Address, DateOfBirth, Gender, Email, Password, PhoneNumber) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
            );
            $stmt->bind_param("sssssssi", $last_name, $first_name, $address, $date_of_birth, $gender, $email, $password, $phone_number);
